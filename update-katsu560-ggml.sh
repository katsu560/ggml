...
FIXBASE="fix"
SCRIPTNAME=ggml
UPDATENAME=update-${GITNAME}-${SCRIPTNAME}.sh
FIXSHNAME=${FIXBASE}[0-9][0-9][01][0-9][0-3][0-9].sh
FIXSHLATESTNAME=${FIXBASE}latest.sh
...
do_bin()
{
	local RETCODE XDOBIN XMODEL VARPROMPT DOOPT XWAVFILE PROMPTTXT XSEED
	...
	if [ x"$XDOBIN" = x"whisper" ]; then
		...
		msg "./$DIRNAME/$XDOBIN -m $XMODEL $DOOPT -f $XWAVFILE"
		if [ $NOEXEC -eq $RET_FALSE ]; then
			./$DIRNAME/$XDOBIN -m $XMODEL $DOOPT -f $XWAVFILE
			RETCODE=$?
		fi
		if [ ! $RETCODE -eq $RET_OK ]; then
...
# func:mk_targets ver: 2024.01.20
...
mk_targets()
{
	...
	msg "cp -p $XBINS $DIRNAME/"
	if [ $NOEXEC -eq $RET_FALSE ]; then
		cp -p $XBINS $DIRNAME || die 282 "can't cp $XBINS"
	fi
}

# func:do_gpt2 ver: 2024.01.20
...
do_gpt2()
{
	...
	if [ $MKCLEAN -eq $RET_FALSE -a $NOCLEAN -eq $RET_FALSE ]; then
		msg "make clean"
		if [ $NOEXEC -eq $RET_FALSE -a ! x"$DOOPT" = x"NOMAKE" ]; then
...
# func:do_whisper ver: 2024.01.20
...
# func:do_gptneox ver: 2024.01.20
...
# func:do_dollyv2 ver: 2024.01.20
...
# func:do_examples ver: 2024.01.20
...
do_examples()
{
	...
	if [ ! x"$EXOPT" = xNOMAKE ]; then
		...
		msg "cp -p $XBINTESTS $DIRNAME/"
		if [ $NOEXEC -eq $RET_FALSE ]; then
			cp -p $XBINTESTS $DIRNAME || die 53 "can't cp $XBINTESTS"
			NOCOPY=$RET_TRUE
		fi
	fi
...
usage()
{
	echo "usage: $MYNAME [-h][-v][-n][-nd][-ncp][-ts][-nc][-noavx|avx|avx2][-lv LEVEL][-s SEED] dirname branch cmd"
	...
	echo "  dirname ... directory name ex. 240120up"
